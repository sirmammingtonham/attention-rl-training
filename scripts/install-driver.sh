...
function install_driver_ubuntu() {
  ...
  sudo apt-get -o DPkg::Lock::Timeout=120 --allow-releaseinfo-change update
  sudo apt-get -o DPkg::Lock::Timeout=120 remove -y "${DRIVER_UBUNTU_PKG}"
  sudo apt-get -o DPkg::Lock::Timeout=120 -y autoremove && sudo apt-get -o DPkg::Lock::Timeout=120 install -y "${DRIVER_UBUNTU_PKG}" nvidia-modprobe
...
main() {
  ...
  if [[ "${OS_IMAGE_FAMILY}" == "${OS_DEBIAN10}" ]]; then
    install_driver_debian
  elif [[ "${OS_IMAGE_FAMILY}" == "${OS_UBUNTU2004}" ]]; then
