    ...
    for algo, name in zip(args.algos, args.legend_map):
        ...
        for folder_idx, exp_folder in enumerate(args.exp_folders):
            ...
            if len(merged_results) > 0:
                ...
                if args.median:
                    results[env][name] = f"{np.median(last_evals):.0f}"
                else:
                    results[env][
                        name
                ...
                post_processed_results[env][name] = {
                    "timesteps": timesteps,
                    "mean": mean_,
                    "std_error": std_error,
                ...
                plt.plot(timesteps / divider, mean_, label=name, linewidth=1.5)
...
for i, env in enumerate(args.env, start=1):
    ...
    for algo, name in zip(args.algos, args.legend_map):
        ...
        for label in args.labels:
            key = name
            value_matrix[i].append(f'{int(max(post_processed_results[env][name]["mean"]))}')
